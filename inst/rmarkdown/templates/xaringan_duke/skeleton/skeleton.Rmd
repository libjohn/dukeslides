---
title: "A xaringan slide template for Duke University"
subtitle: "combines CSS styles"
author: "John Little"
date: "`r Sys.Date()`"
output:
...
This example slide deck extends the excellent collaborative work of the Xaringan and Remark.js slide tools.  With this template, slide authors can compose xaringan slides using [Rmarkdown](https://rmarkdown.rstudio.com/) to present [Duke University branding](https://styleguide.duke.edu/color-palette/).

[Yihui Xie](https://yihui.name/) provides wonderful [technical documentation](https://slides.yihui.name/xaringan/) and a [wiki](https://github.com/yihui/xaringan/wiki) for the [xaringan package](https://github.com/yihui/xaringan). Some of his documentation is repeated here.  **My goal** is to present **example slides** with example **RMarkdown** and xaringan **syntax** to demonstrate presentation styles. 

---
# NOTE

Most of the cheatsheet examples will get displayed in the slide itself.  Some of the example code will be found in the presenter notes.  The full detailed R Markdown can always be displayed by opening the rendered HTML slides in your web browser and `View Source`.  You can access the presenter notes by pressing the keyboard shortcut `p`.  (`?` for other shortcuts)
...
- Duke color palette overlayed on Hygge CSS, (`hygge-duke.css`)
- Creative Commons licensing fonts (CC BY-NC, etc.) (`cc-fonts.css`)
- some figure Captions (`figures.css`)

You can find these style sheets declared in the `yaml` header of the Rmarkdown slide file.
...
Two left-justified dashes `--` creates a pause to reveal the slide in increments.  `click` or `arrow` to advance just likes from slide to slide. [more documentation on incrementing](https://slides.yihui.name/xaringan/incremental.html#11)
...
```
list items (bullets) are invoked with a leading, left-justified dash '-' 
(just like any Rmarkdown document)
...
.black[See presenter mode, `p`, to see how background-images are declared]
...
See presenter mode, `p`, to see how background-images are declared
...
I think the term "template" is ambiguous here.  You can follow the documententation.  There are RMarkdown templates and then there are Remark.js templates.  
